...
def _make_step(
        net, step_size=1.5, end="inception_4c/output", jitter=32, clip=True, verbose=False):
  """ Basic gradient ascent step. """

  if verbose:
    print("End layer is %s " % end)
...
def _deepdream(
        ...
        end="inception_4c/output", clip=True, verbose=False, **step_params):
  ...
  for octave, octave_base in enumerate(octaves[::-1]):
    ...
    for i in xrange(iter_n):
      _make_step(net, end=end, clip=clip, verbose=verbose, **step_params)
